...
import Hadjj from "@/models/Hadjj"
...
export default {
  ...
  data () {
    return {
      ...
      e3: true,
      hadjj: new Hadjj(this.$store.state.firestore.collection("haddjes")).init()
    };
  },
  computed: {},
  methods: {
    ...
    register () {
      ...
      this.$auth
        ...
        .then(user => {
          this.ready = false;
          this.hadjj.email = user.user.email
          this.hadjj.uid = user.user.uid    
          this.hadjj.add()
    ...
    signInGoogle () {
      ...
      return this.$auth
        .signInWithGoogle()
        .then(result => {
          this.hadjj.email = result.result.email
          this.hadjj.uid = result.result.uid
          this.hadjj.add()
        })
        .catch(error => {
          this.error = error.message;
